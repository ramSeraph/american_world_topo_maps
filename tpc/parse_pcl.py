...
from parse_mapstor import TPCMapstorProcessor
...
class TPCPCLProcessor(TPCMapstorProcessor):

    def __init__(self, filepath, extra, index_box, index_properties, projinfo=None, id_override=None):
        super().__init__(filepath, extra, index_box, index_properties, projinfo, id_override)
    ...
    def rotate(self):
        if self.inset_pixel_cutlines is None:
            return

        full_img_path = self.get_full_file_path()
        full_img = Image.open(full_img_path)
        full_img = self.inpaint_insets(full_img, self.inset_pixel_cutlines, self.inpaint_color)

        workdir = self.get_workdir()
        workdir.mkdir(parents=True, exist_ok=True)
        full_img_path = workdir / 'full.jpg'
        full_img.save(full_img_path, format='JPEG', subsampling=0, quality=100)
    ...
    for filepath in image_files:
        ...
        for subid, subextra in subs:
            ...
            for item_id, item_extra in all_items:
                processor = TPCPCLProcessor(filepath, item_extra, [], sheet_props, projinfo, item_id)
